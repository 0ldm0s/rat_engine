//! 双端口模式 - gRPC TLS + HTTP 明文
//!
//! - HTTP 端口: 3001 (明文，无需证书)
//! - gRPC 端口: 50051 (TLS，需要证书)
//!
//! 适用场景：
//! - 内部服务调用使用明文 HTTP
//! - 外部 gRPC 客户端使用加密连接
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    println!("🚀 RAT Engine 双端口模式 - gRPC TLS + HTTP 明文");
    println!("================================================");
    println!("HTTP 端口: http://localhost:3001 (明文)");
    println!("gRPC 端口: https://ligproxy-test.0ldm0s.net:50051 (TLS)");
    println!();

    // 配置分端口模式证书：gRPC 有证书，HTTP 无证书
    ...
    // 使用 separated 模式：gRPC 证书，HTTP 无证书（None）
    let cert_manager_config = CertManagerConfig::separated(grpc_cert_config, None);
...
<html>
<head>
    <title>RAT Engine 双端口模式 - gRPC TLS + HTTP 明文</title>
    <meta charset="utf-8">
</head>
<body>
    <h1>🚀 RAT Engine 双端口模式</h1>
    <h2>gRPC TLS + HTTP 明文</h2>
    <p><strong>HTTP 服务器</strong>: 运行在端口 3001（明文，无需 TLS）</p>
    <p><strong>gRPC 服务器</strong>: 运行在端口 50051（TLS 加密）</p>
    <h3>适用场景</h3>
    <ul>
        <li>内部服务调用使用明文 HTTP（简单、高效）</li>
        <li>外部 gRPC 客户端使用加密连接（安全）</li>
    </ul>
</body>
</html>
"#,
    ...
    router.add_route(rat_engine::Method::GET, "/health", |_req| {
        Box::pin(async {
            Ok(Response::builder()
                ...
                .body(Full::new(hyper::body::Bytes::from(
                    r#"{"status": "ok", "http_mode": "plaintext", "grpc_mode": "tls"}"#,
                )))
                .unwrap())
        })
    });
    ...
    println!();
    println!("测试方法:");
    println!("  HTTP (明文): curl http://localhost:3001/");
    println!("  HTTP (明文): curl http://localhost:3001/health");
    println!("  gRPC (TLS):  运行 gRPC 客户端连接 ligproxy-test.0ldm0s.net:50051");
    println!();
    println!("按 Ctrl+C 停止");
